...
var props = defineProps(["programStudi"]);
...
const formStatusSubmit = () => {
    form.put(route('prodis.update', props.programStudi.id))
};
...
<AppHead :title="'Edit programStudi '+programStudi.id" />
    <LayoutAuthenticated>
        
        <SectionMain>
            <SectionTitle>Edit programStudi {{ programStudi.id }}</SectionTitle>
            ...
            <CardBox class="md:w-7/12 lg:w-5/12 xl:w-4/12 shadow-2xl md:mx-auto" is-form is-hoverable
                ...
                <NotificationBarInCard :color="getFormStatusColor" :is-placed-with-header="formStatusWithHeader">
                    <span><b class="capitalize">{{
                        ...
                        {{form.recentlySuccessful?" Berhasil menambahkan": "Tambah Program Studi"}}</span>
                </NotificationBarInCard>
                <FormField label="Name">
                    <FormControl v-model="form.name"  :icon-left="mdiAccount" help="Program Studi" placeholder="Program Studi"
                        required />
                </FormField>
